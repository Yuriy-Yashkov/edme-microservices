...
import ru.edme.issuing.dto.CardTransferDto;
...
public class CardProducerServiceImpl implements CardProducerService {
    ...
    @Value("${spring.kafka.topics.card-transfer}")
    ...
    private CardTransferDto mapperToCardTransferDto(CardDto cardDto, LocalDateTime dateTime) {
        return new CardTransferDto(
                ...
                cardDto.getExpirationDate(), cardDto.getHolderName(),
                cardDto.getCardStatus().getId(), cardDto.getPaymentSystem().getId(),
                cardDto.getAccount().getId(), dateTime, 1L
        );
    }
